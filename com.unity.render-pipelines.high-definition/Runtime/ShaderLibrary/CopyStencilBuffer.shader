Shader "Hidden/HDRP/CopyStencilBuffer"
{
    ...
    RW_TEXTURE2DX(float, _HTile) : register(u1); // DXGI_FORMAT_R8_UINT is not supported by Unity
    RW_TEXTURE2DX(float, _StencilBufferCopy) : register(u1); // DXGI_FORMAT_R8_UINT is not supported by Unity
#else
    RW_TEXTURE2DX(float, _HTile); // DXGI_FORMAT_R8_UINT is not supported by Unity
    RW_TEXTURE2DX(float, _StencilBufferCopy); // DXGI_FORMAT_R8_UINT is not supported by Unity
#endif
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void Frag(Varyings input) // use SV_StencilRef in D3D 11.3+
            {
                ...
                _HTile[COORD_TEXTURE2DX(positionNDC / 8)] = _StencilRef;
            }
        ...
        Pass
        {
            ...
            #pragma fragment Frag
                ...
                {
                    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
                    _StencilBufferCopy[COORD_TEXTURE2DX((uint2)input.positionCS.xy)] = PackByte(1);
                }
        ...
        Pass
        {
            ...
            #pragma fragment Frag
                ...
                void Frag(Varyings input) // use SV_StencilRef in D3D 11.3+
                {
                    ...
                    uint oldStencilVal = UnpackByte(_StencilBufferCopy[COORD_TEXTURE2DX(dstPixCoord)]);
                    _StencilBufferCopy[COORD_TEXTURE2DX(dstPixCoord)] = PackByte(oldStencilVal | _StencilRef);
                }
