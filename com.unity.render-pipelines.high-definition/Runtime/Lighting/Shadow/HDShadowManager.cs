...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public class HDShadowManager : IDisposable
    {
        ...
        public void DisplayShadowAtlas(CommandBuffer cmd, HDCamera hdCamera, Material debugMaterial, float screenX, float screenY, float screenSizeX, float screenSizeY, float minValue, float maxValue, bool flipY)
        {
            m_Atlas.DisplayAtlas(cmd, hdCamera, debugMaterial, new Rect(0, 0, m_Atlas.width, m_Atlas.height), screenX, screenY, screenSizeX, screenSizeY, minValue, maxValue, flipY);
        }

        // Warning: must be called after ProcessShadowRequests and RenderShadows to have valid informations
        public void DisplayShadowCascadeAtlas(CommandBuffer cmd, HDCamera hdCamera, Material debugMaterial, float screenX, float screenY, float screenSizeX, float screenSizeY, float minValue, float maxValue, bool flipY)
        {
            m_CascadeAtlas.DisplayAtlas(cmd, hdCamera, debugMaterial, new Rect(0, 0, m_CascadeAtlas.width, m_CascadeAtlas.height), screenX, screenY, screenSizeX, screenSizeY, minValue, maxValue, flipY);
        }

        // Warning: must be called after ProcessShadowRequests and RenderShadows to have valid informations
        public void DisplayShadowMap(int shadowIndex, CommandBuffer cmd, HDCamera hdCamera, Material debugMaterial, float screenX, float screenY, float screenSizeX, float screenSizeY, float minValue, float maxValue, bool flipY)
        {
            ...
            if (shadowRequest.allowResize)
                m_Atlas.DisplayAtlas(cmd, hdCamera, debugMaterial, shadowRequest.atlasViewport, screenX, screenY, screenSizeX, screenSizeY, minValue, maxValue, flipY);
            else
                m_CascadeAtlas.DisplayAtlas(cmd, hdCamera, debugMaterial, shadowRequest.atlasViewport, screenX, screenY, screenSizeX, screenSizeY, minValue, maxValue, flipY);
        }
